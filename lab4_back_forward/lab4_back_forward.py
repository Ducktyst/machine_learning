...
1. Модифицируйте сеть из листинга 59. Добавьте еще один слой
и восстановите работоспособность алгоритма. Критерием его работо-
способности может быть стремительно уменьшающаяся ошибка.
2. Дополнительно обучите старую и новую модели на следу-
ющих данных:
X = [[0, 0, 1], [0.3, 1, 0],
[1, 0.3, 0], [0.6, 0.2, 1],
[0.6, 0.2, 1]]
Проанализируйте исходные данные. Какая зависимость скрыта
в них? Объясните, что произошло с качеством классификации.
Почему? С чем может быть связано подобное в реальных задачах
и как на это можно повлиять (если это вообще возможно)?
"""
...
# W_2_3 - это матрица весов между вторым и третьим слоем.
# Все остальное как в W_1_2
...
for jj in range(100000):
    ...
    # На пересечении i-й строки и j-го столбца получаем ячейку с конкретной
    # взвешенной суммой для i-го входного образа и j-го нейрона слоя (l2).
    # Иными словами, для каждого нейрона слоя L2 и для каждого входа
    ...
    # По сути l3 это матрица 4 на 1, где в каждой ячейке
    # результат активации нейрона третьего слоя (а он один)
    ...
    # l3_sigma рассчитывается как ошибка выхода всей сети
    # на производную функции активации всех нейронов L3.
    # На пересечении i-й строки и j-го столбца получаем ячейку
    # с конкретной сигмой для
    # i-го входного образа и j-го нейрона слоя (l3).
    # Иными словами, для каждого нейрона слоя L3
    # и для каждого входа рассчитывается производная по функции
    # активации умноженная на ошибку.
    # То есть l3_sigma будет матрица 4 на 1 (поскольку
    # в L3 только один нейрон).
    l3_sigma = l3_error * sigma_derivative(l3)
    print(l3_sigma)

    # Ошибка L2 слоя оценивается через взвешенную сигму
    # слоя L3 по весам между L2 и L3
    # Поскольку l3_sigma это матрица 4 на 1 и матрица W_2_3 4 на 1,
    # то последнюю матрицу надо Транспонировать, чтобы выполнить
    # правило умножения матриц и взвесить элементы l3_sigma
    # по элементам W_2_3.
    # Тогда итоговая матрица l1_error будет 4 на 4,
    # где на пересечении i-й строки и j-го столбца
    # будет ячейка с конкретной ошибкой j-го нейрона слоя L2
    # для i-го входного образа.
    ...
    # каждом столбце l3_sigma будет по одному значению,
    # а в каждой строке l2 по 4 значения, а значит
